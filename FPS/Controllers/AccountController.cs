...
namespace FPS.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            using (var uow = new FPSDatabaseEntities())
            {
                ...
                var userDataTemp =
                    uow.AspNetUsers.FirstOrDefault(
                        st => st.UserName == emailID && st.PasswordHash == password);

                if (userDataTemp != null)
                {
                    CacheManager.UserData = userDataTemp;
                    CacheManager.LoadSystemDetails();
                    CacheManager.RenewCurrentUser();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                //var user = new ApplicationUser {   LastName=model.Lname,CType=model.Ctype };
                ...
                try
                {
                    using (var uow = new FPSDatabaseEntities())
                    {
                        var userData =
                        uow.AspNetUsers.FirstOrDefault(
                            st => st.Email.Equals(model.Email, StringComparison.InvariantCultureIgnoreCase));

                        if (userData == null)
                        {
                            var newAspNetUsers = new AspNetUser
                            {
                            ...
                            uow.AspNetUsers.Add(newAspNetUsers);
                            uow.SaveChanges();
                            userData = newAspNetUsers;
                            FormsAuthentication.SetAuthCookie(model.Email, true);
                        ...
                        {
                            ViewBag.errormsg = "Email already registered. Please try with another.";
                        }
                    }
                }
                catch (Exception ex)
                {
                    ViewBag.errormsg = "Please try again.";
                    return View(model);
                }               
            }
